---
title: Configuration
description: Complete reference for cortex.config.yml and vendor extensions.
order: 2
---
...
| `type`      | string | Yes      | One of: `openapi-spec`, `asyncapi-spec`, `graphql-spec`, `grpc-spec`    |
...
| `grpc-spec`     | Protocol Buffer file (gRPC service)                 |
...
Multiple sources can share the same `package_name` and `github_repository`. When they do, all protocol-specific code (REST, WebSocket, GraphQL, gRPC) is merged into a single SDK package:
...
- **Local file paths**: `./openapi.yaml`, `../specs/api.json`
...
- **gRPC services** from gRPC sources

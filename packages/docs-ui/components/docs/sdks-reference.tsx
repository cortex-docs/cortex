...
const LANG_META: Record<string, { icon: string; color: string; bg: string; accent: string }> = {
  typescript: {
    ...
    color: 'text-[#3178c6]',
    bg: 'bg-[#3178c6]/10',
    accent: 'border-[#3178c6]/30',
  },
  python: {
    icon: 'PY',
    color: 'text-[#3776ab]',
    bg: 'bg-[#3776ab]/10',
    accent: 'border-[#3776ab]/30',
  },
  ...
  java: {
    ...
    color: 'text-[#f89820]',
    bg: 'bg-[#f89820]/10',
    accent: 'border-[#f89820]/30',
  },
  kotlin: {
    icon: 'KT',
    color: 'text-[#7f52ff]',
    bg: 'bg-[#7f52ff]/10',
    accent: 'border-[#7f52ff]/30',
  },
  ruby: {
    icon: 'RB',
    color: 'text-[#cc342d]',
    bg: 'bg-[#cc342d]/10',
    accent: 'border-[#cc342d]/30',
  },
  php: {
    icon: 'PHP',
    color: 'text-[#777bb4]',
    bg: 'bg-[#777bb4]/10',
    accent: 'border-[#777bb4]/30',
  },
  csharp: {
    icon: 'C#',
    color: 'text-[#68217a]',
    bg: 'bg-[#68217a]/10',
    accent: 'border-[#68217a]/30',
  },
  rust: {
    icon: 'RS',
    color: 'text-[#dea584]',
    bg: 'bg-[#dea584]/10',
    accent: 'border-[#dea584]/30',
  },
  cpp: {
    icon: 'C++',
    color: 'text-[#00599c]',
    bg: 'bg-[#00599c]/10',
    accent: 'border-[#00599c]/30',
  },
...
export function SdksReference({ initialLanguage }: { initialLanguage?: string }) {
  ...
  useEffect(() => {
    const language = pathname?.match(/^\/sdks\/([^/]+)$/)?.[1];
    if (!language) return;

    const decodedLanguage = decodeURIComponent(language);
    if (packages.some((pkg) => pkg.language === decodedLanguage)) {
      setActiveLang(decodedLanguage);
      setSelectedPkg(null);
    }
  }, [packages, pathname]);
  ...
  function selectLang(lang: string) {
    ...
    const nextPath = `/sdks/${encodeURIComponent(lang)}`;
          ...
          segments={[
            ...
            ...(activeLang
              ? [
                  {
                    label:
                      uniqueLangs.find((p) => p.language === activeLang)?.languageDisplayName ??
                      activeLang,
                  },
                ]
              : []),
          ...
          <>
            ...
            <div className="mx-auto max-w-4xl px-6 py-8 lg:px-10">
              <div
                ...
                dangerouslySetInnerHTML={{ __html: readmes[activeLang!] ?? '' }}
              />
            </div>
          </>
        ) : (
              ...
              {activePkgs.map((pkg) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {pkg.protocols.map((p) => (
                          <span
                            key={p}
                            className="inline-flex items-center rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground uppercase tracking-wider"
                          >
                            {p}
                          </span>
                        ))}
                    ...
                    {pkg.githubRepository && (
                      <a
                        href={pkg.githubRepository}
                        target="_blank"
                        rel="noreferrer"
                        aria-label={`Open ${pkg.packageName} source repository`}
                        title="Open source repository"
                        className="absolute top-4 right-4 rounded p-1 text-muted-foreground opacity-70 transition-opacity hover:bg-muted hover:text-foreground group-hover:opacity-100"
                      >
                        <svg
